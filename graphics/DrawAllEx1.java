...
public class DrawAllEx1 extends MFrame implements ActionListener {
	...
	class MCanvas extends Canvas {
		...
		public void paint(Graphics g) {
			...
			int[] tf1 = new int[4];
			for (int i = 0; i < tf.length; i++) {
				tf1[i] = Integer.valueOf(tf[i].getText());
			}
			// color : 0(R), 1(G), 2(B) 셋팅
			switch (color) {
			case 0:
			...
			switch (mode) {
			case 0:
				g.drawLine(tf1[0], tf1[1], tf1[2], tf1[3]);
				break;
			case 1:
				if (fillcb.getState())
					g.fillRect(tf1[0], tf1[1], tf1[2], tf1[3]);
				else
					g.drawRect(tf1[0], tf1[1], tf1[2], tf1[3]);
				break;

			case 2:
				if (fillcb.getState())
					g.fillOval(tf1[0], tf1[1], tf1[2], tf1[3]);
				else
					g.drawOval(tf1[0], tf1[1], tf1[2], tf1[3]);
				break;
			}
		}
	}
